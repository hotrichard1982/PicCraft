...
- 早期分析（**2026-08-14 已证伪**）：曾指向 **image-0.25.10 BMP encoder**（`bmp/encoder.rs:71` 的同类断言，经 `#[track_caller]` 显示为调用点 `io/encoder.rs:115`）。**证伪理由**：BMP 编码只出现在保存操作（`save_image`/`batch`/`save_to_temp`），选文件夹不触发；且当时复现矩阵未覆盖 CMYK JPEG（image crate 无法编码该格式，需外部生成）。**真根因见下方「回执」：CMYK JPEG → 缩略图 PNG 编码断言，本地 catch_unwind 精确复现，与真机日志逐字节一致。**
- 传播路径：主线程（macOS Tauri IPC 经 WKWebView scheme handler `start_task`）→ Rust panic 跨 FFI → SIGABRT。`.ips` 的 `start_task` 栈是传播路径，非根因；TCC/内存是背景。

## 附加防御修复（2026-08-13，保留）

- `src-tauri/src/image_ops.rs` 三处 BMP 编码路径显式 `to_rgb8()`：`save_image` bmp 分支、`process_single_batch` bmp 分支、`save_to_temp`（无害的附加防御，与 CMYK 修复互补）。
- 新增 2 个回归测试：`test_save_bmp_rgba_source_ok`、`test_save_to_temp_bmp_rgba_ok`（RGBA 源 → BMP roundtrip）。
- 验证：`cargo test --locked` 通过（含该修复）；`git diff --check` 通过。
