...
- **决定性证据（2026-08-13）**：`piccarft.log` 显示 panic 为 **image-0.25.10 BMP encoder buffer 长度断言**（expected 55890=Rgb8 / got 74520=RGBA for 207x90），非 wry unwrap。`start_task` 只是 macOS IPC/asset 的 panic 传播线程。
...
- **取证完成后的结论（2026-08-13）**：真根因为 BMP encoder color/buffer 不一致断言（image 0.25.10 特定输入下 DynamicImage 状态异常）。**不升级 tauri/wry、不改大图加载**，采用**防御性修复**：`image_ops.rs` 三处 BMP 编码路径显式 `to_rgb8()`，消除断言路径。已实施并全量回归（Rust 74 passed）。
